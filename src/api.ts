...
export default class ApiManager {
  ...
  async getNotebooks() {
    ...
    try {
      ...
      if (e.status == 401) {
        console.log(
          `parse request to cURL for debug: ${this.parseToCurl(req)}`
        );
        await this.refreshCookie();
      }
    }
  ...
  private parseToCurl(req: RequestUrlParam) {
    ...
    Object.keys(requestHeaders).forEach((name) => {
      command.push("-H");
      command.push(
        this.escapeStringPosix(
          name.replace(/^:/, "") + ": " + requestHeaders[name]
        )
      );
    });
  ...
  private updateCookies(respCookie: string) {
    ...
    cookies.forEach((cookie) => {
      const newCookie = refreshCookies.find(
        (freshCookie) => freshCookie.name == cookie.name
      );
      if (newCookie) {
